...
Message DTO может содержать optional closed `clarification_response` с exact
`clar_<opaque>` handle и action `choose|narrow|cancel`; `choice_id` обязателен
только для `choose`. Claim выполняется до turn/external calls и имеет
deterministic one-use/session/version/expiry/catalog/marker errors из
`docs/testing/slice3_acceptance_contract.md`. Отдельный closed
`context_action={kind:remove,handle}` инвалидирует только названный active slot
без DeepSeek/MCP. `GET /sessions/{id}` возвращает active slot summary только как
handle, slot key, semantic/value type, policy mode, cardinality, member count,
presentation и expiry; UUID/physical/full refs и raw scalar values остаются
server-side.
