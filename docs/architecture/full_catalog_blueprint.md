...
| M06 | `count_distinct` и `aggregate` | count/sum/avg/min/max | Operand semantic type, distinct key, unit и denominator обязательны |
...
| R06 | `ut115.ref.warehouse.resolve` | name/type/organization/purpose | warehouse ref and selection attributes |
...
operation. R01A/R01B/R01C use equality and can never degrade to prefix/contains.
...
| SP04 | `ut115.sales.shipment-list` | shipment documents with amount/status and refs | `CAP-SALES-SHIPMENT-LIST` |
| SP05 | `ut115.sales.shipment-lines` | lines of exact shipment ref | `CAP-SALES-SHIPMENT-LINES` |
| SP06 | `ut115.sales.performance` | quantity, revenue, cost, gross profit on identical coordinates | `CAP-SALES-TURNOVER`, `CAP-SALES-PROFIT`, `CAP-FIN-REVENUE`, `CAP-FIN-COST`, `CAP-FIN-PROFIT` |
...
| P03 places/money refs | `Catalogs/Склады.xml` (`ТипСклада`, retail/storage attributes); `Catalogs/Кассы.xml`; `Catalogs/КассыККМ.xml` (`ТипКассы`, `Склад`, currency); `Catalogs/Организации.xml`; `Catalogs/БанковскиеСчетаОрганизаций.xml` | R06-R09, CF01-CF02 |
...
- keyset/prefix pagination contract, default page 20, page-size+1 detection,
  opaque server-side continuation handle and explicit continuation message;
- stage deadlines/retries, dependency diagnostics and manual session/trace clear;
- finish SP01's full header/status contract; add R06 and SP04 because Q054 and
  Q031/Q103 need real producers, not fixture-only application branches;
...
- Q012,Q015,Q031,Q054,Q099,Q102-Q106 pass on fixtures; selected positive/empty
  queries pass live when MCP is available;
- cursor is bound to session, turn, skill/version, params, marker and sort tuple;
...
- AC-018, AC-021, AC-024, AC-046, AC-049 and AC-050 gates are green.
