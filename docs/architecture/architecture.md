...
2. В runtime выполняется только фиксированный query package из принятой ревизии
   навыка: один SELECT либо объявленный связанный temp-table batch по ADR-0003;
   business-instance values передаются как параметры, literals типизированы.
...
- модели навыка, query package/execution manifest, semantic output contract и плана;
...
- `ImportSkillDocument` (bare skill или package), `ReplaceSkill`, `DeleteSkill`,
  `ExportSkill`;
...
| Query template | Неизменяемый параметризованный query package внутри data-query skill с закрытым `single_select`/`linked_temp_batch` execution contract | Не генерируется и не редактируется DeepSeek в runtime; linked batch не является composite plan |
...
1. Query execution: фиксированный package, typed bindings/invariant constants,
   связанный temp-table graph и exact final column map.
...
проверяет coverage, связывает параметры, выполняет query package, классифицирует
